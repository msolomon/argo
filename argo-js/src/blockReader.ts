...
export class LabelBlockReader<Out> extends BlockReader<Out> {
  constructor(public buf: BufRead, protected fromBytes: (bytes: Uint8Array) => Out) {
    super(buf)
  }
  ...
  read(parent: BufRead): Out | undefined | null {
    ...
    switch (Label.kind(label)) {
      case LabelKind.Backreference:
        throw 'Programmer error: This type must not use backreferences'
      ...
      case LabelKind.Absent:
        throw 'Programmer error: Reader cannot handle absent labels'
      case LabelKind.Error:
        throw 'Programmer error: Reader cannot handle error labels'
    }
  }
}
...
export class FixedSizeBlockReader<Out> extends BlockReader<Out> {
  ...
  read(parent: BufRead): Out {
    const bytes = this.buf.read(this.byteLength)
    return this.fromBytes(new Uint8Array(bytes, bytes.byteOffset, this.byteLength)) // aligned reads
  }
}
