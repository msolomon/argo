...
async function* loadTests(dir = path.dirname(testPath)) {
    let count = 0;
...
async function* loadTest(dir) {
    ...
    for await (const p of walk(dir)) {
        ...
        else if (p.includes('disabled'))
            continue;
        else
            throw `Got unexpected file in test: ${p}`;
    }
...
async function* walk(dir, dirsOnly = false) {
...
function capnpPackedLength(bytes) {
...
async function runEquivalence(name, query, json, schema, expected) {
    ...
    const savedWithArgo = (json, argo) => `${(json - argo).toLocaleString("en-US")} bytes (${100 - Math.round(argo / json * 100)}%)`;
    const sizes = {
    ...
    const smallest = Object.entries(sizes).reduce(([lastName, last], [nextName, next]) => {
        if (Math.min(last.json, last.argo) <= Math.min(next.json, next.argo))
