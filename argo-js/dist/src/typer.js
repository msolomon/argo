...
class Typer {
    ...
    static directives = [ArgoCodecDirective, ArgoDeduplicateDirective];
    ...
    get rootType() {
        ...
        if (!type)
            throw "Constructed without root type";
        return type;
    }
    rootWireType() {
    ...
    collectFieldsStatic = (selectionSet, visitedFragments = new Set()) => {
        const groupedFields = new Map();
        const getGroupedField = (responseKey) => {
            if (!groupedFields.has(responseKey))
                groupedFields.set(responseKey, []);
        ...
        for (const selection of selectionSet.selections) {
            ...
            selection.directives?.some(dn => dn.name.value == 'skip' &&
                dn.arguments?.some(an => an.value.value == true)))
                continue;
            ...
            selection.directives?.some(dn => dn.name.value == 'include' &&
                dn.arguments?.some(an => an.value.value == false)))
                continue;
            if (selection.kind == Kind.FIELD) {
                ...
                if (fragment == null)
                    throw `Referenced Fragment did not exist: ${fragmentSpreadName}`;
    ...
    static unwrap(t) {
        if (graphql.isWrappingType(t))
            return Typer.unwrap(t.ofType);
        else
            return t;
    }
    /**
     ...
     */
    collectFieldWireTypes = (selectionType, selectionSet, getField) => {
        let recordFields = [];
        const recordNodes = [];
        for (const [alias, fields] of this.collectFieldsStatic(selectionSet)) {
            for (const { field, selectedBy } of fields) {
                ...
                if (field.selectionSet) {
                    ...
                    const wfield = { name: alias, type, omittable };
    ...
    groupOverlapping(fields) {
        ...
        if (Array.from(grouped.values()).some(g => g.length > 1)) { // need to merge overlapping fields
            ...
            for (const [name, fields] of grouped) {
                ...
                if (fields.length == 1)
                    ...
                    const type = wrapRecord(this.groupOverlapping(combinedFields));
        ...
        const record = { type: Wire.TypeKey.RECORD, fields: recordFields };
        return record;
    }
    ...
    typeToWireType = (t) => {
        if (graphql.isScalarType(t) || graphql.isEnumType(t)) {
            ...
            const mkBlockType = (type) => Wire.block(codec ?? type, t.name, deduplicate ?? Wire.deduplicateByDefault(type));
            if (graphql.isEnumType(t)) {
                ...
                switch (t) {
                    ...
                    case graphql.GraphQLBoolean:
                        if (deduplicate)
                            throw 'Boolean fields cannot be deduplicated';
                        wtype = Wire.BOOLEAN;
                        break;
                    default:
                        if (codec == null)
                            throw 'Custom scalars must have a ArgoCodec directive. Missing on ' + t.name;
    ...
    unwrapForSelectionSet(wt) {
        if (Wire.isRECORD(wt)) {
            return { record: wt, wrap: (wt) => wt };
        }
        else if (Wire.isNULLABLE(wt)) {
            const { record, wrap } = this.unwrapForSelectionSet(wt.of);
            return { record, wrap: (r) => Wire.nullable(wrap(r)) };
        }
            ...
            const { record, wrap } = this.unwrapForSelectionSet(wt.of);
            return { record, wrap: (r) => { return { type: Wire.TypeKey.ARRAY, of: wrap(r) }; } };
        }
        else {
