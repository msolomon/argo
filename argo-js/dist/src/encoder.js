...
class ArgoEncoder {
    ...
    static utf8encode = this.utf8.encode.bind(this.utf8);
    writers = new Map();
    tracked = [];
    header;
    DEBUG = false; // set to true to enable tracking of extra information
    constructor(buf = new Buf()) {
        this.buf = buf;
    ...
    log = (msg) => {
        if (this.DEBUG) {
            if (typeof msg === 'string')
                this.tracked.push({ pos: this.buf.position, msg });
            else
                this.tracked.push({ pos: this.buf.position, ...msg });
        }
    };
    getResult() {
        ...
        if (buf.length != buf.capacity)
            throw 'Programmer error: incorrect result length ' + buf.length + ', expected ' + buf.capacity;
        return buf;
    }
    static NullTerminator = new Uint8Array([0]);
    makeBlockWriter(t, dedupe) {
        switch (t.type) {
            case "STRING":
                let writer;
                if (dedupe)
                    writer = DeduplicatingBlockWriter.lengthOfBytes(ArgoEncoder.utf8encode);
                else
                    writer = BlockWriter.lengthOfBytes(ArgoEncoder.utf8encode);
            ...
            case "BYTES":
                if (dedupe)
                    return DeduplicatingBlockWriter.lengthOfBytes(bytes => bytes);
                else
                    return BlockWriter.lengthOfBytes(bytes => bytes);
            case "VARINT":
                if (dedupe)
    ...
    write(block, t, v) {
        const writer = this.getWriter(block, t);
        ...
        if (this.header.inlineEverything) { // write to buf instead of block
            if (writer.valuesAsBytes.length == blockLengthBefore + 1)
                writer.writeLastToBuf(this.buf);
        }
        return label;
    }
    getWriter(block, t) {
        ...
        switch (wt.type) {
            case 'NULLABLE':
                ...
                if (js instanceof Error) {
                    ...
                    if (!this.header.selfDescribingErrors)
                        throw 'Unimplemented: non-self-describing errors';
