import { DirectiveLocation, GraphQLBoolean, GraphQLDirective, GraphQLEnumType, GraphQLInt, GraphQLNonNull } from "graphql";
...
export var ArgoCodec;
(function (ArgoCodec) {
    ArgoCodec["String"] = "String";
    ArgoCodec["Int"] = "Int";
    ArgoCodec["Float"] = "Float";
    ArgoCodec["Boolean"] = "Boolean";
    ArgoCodec["BYTES"] = "BYTES";
    ArgoCodec["FIXED"] = "FIXED";
})(ArgoCodec || (ArgoCodec = {}));
...
export function getArgoCodecDirectiveValue(node) {
    ...
    else if (fixedLengthNode?.kind == 'IntValue')
        length = parseInt(fixedLengthNode.value);
    else
        throw 'Invalid fixedLength kind on ArgoCodecDirective: ' + fixedLengthNode?.kind;
    let wire;
    switch (codec) {
        ...
        case ArgoCodec.FIXED:
            if (length == null)
                throw 'fixedLength argument is required on ArgoCodecDirective for FIXED codec';
...
export function getArgoDeduplicateDirectiveValue(node) {
    ...
    else if (deduplicateNode?.kind == 'BooleanValue')
        deduplicate = deduplicateNode.value;
    else
        throw 'Invalid deduplicate value on ArgoCodecDirective';
}
